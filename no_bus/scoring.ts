...
function scoreCheckboxes(q: Extract<Question, { type: 'checkboxes' }>, a: Answer): number {
  ...
  // Separate positive and negative weights to avoid breaking normalization
  const totalPositive = q.options.reduce((s, o) => s + Math.max(0, o.weight ?? 1), 0);
  const selectedPositive = q.options
    .filter(o => ans.selected.includes(o.id))
    .reduce((s, o) => s + Math.max(0, o.weight ?? 1), 0);
  const selectedNegativeAbs = q.options
    .filter(o => ans.selected.includes(o.id))
    .reduce((s, o) => s + Math.max(0, -(o.weight ?? 0)), 0);

  // Check minimum selection requirement
  const minOk = q.minSelect ? ans.selected.length >= q.minSelect : true;
  const base = totalPositive > 0 ? (selectedPositive - selectedNegativeAbs) / totalPositive : 0;
  const normalized = minOk ? Math.max(0, Math.min(1, base)) : 0;
